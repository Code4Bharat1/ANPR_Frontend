...
const PMSites = () => {
  ...
  const fetchSiteDetails = async (siteId) => {
    try {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/project/sites/${siteId}`,
        {
          headers: {
      ...
      setSelectedSite(res.data); // 🔥 FULL DETAILS
    } catch (err) {
  ...
  await fetchSiteDetails(site.id); // 🔥 FULL DETAILS API
};
          ...
          <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start justify-between">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${selectedSite.status === 'Active'
                    ? 'bg-green-100 text-green-700'
                      ...
                      {selectedSite.liveVehicles.map((vehicle, index) => (
                        <div key={index} className="bg-white rounded-lg p-3 border border-gray-200">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${vehicle.status === 'Working' ? 'bg-green-100 text-green-700' :
